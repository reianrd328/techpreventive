...
def save_entry():
    ...
    try:
        ...
        """
        params = (
            data.get("business_name") or data.get("issued_company_owned"),
...
def export_csv():
    ...
    try:
        ...
        # Safe COALESCE strategy: matches records on date_visited. If NULL, evaluates date_issued. 
        # If that's also missing, treats it as today's date (CURDATE()) to ensure it pops up in recent filters.
        sql = """
        ...
        if not rows:
            return jsonify({"success": False, "error": "Filter applied, but no matching logs found."}), 400
